...
const CdeReferenceView = ({ schemaFile, schemaData: schemaDataProp }) => {
    const [schema, setSchema] = useState(null);
    const [loading, setLoading] = useState(!schemaDataProp);
    const [error, setError] = useState(null);
    const [searchTerm, setSearchTerm] = useState('');

    useEffect(() => {
        if (schemaDataProp) {
            setSchema(schemaDataProp);
            setLoading(false);
            setError(null);
            return;
        }
